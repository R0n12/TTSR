...
class Trainer():
    def __init__(self, args, logger, dataloader, model, loss_all):
        ...
        self.params = [
            {"params": filter(lambda p: p.requires_grad, self.model.MainNet_NAS.parameters() if 
             args.num_gpu==1 else self.model.module.MainNet_NAS.parameters()),
             "lr": args.lr_rate
            },
            {"params": filter(lambda p: p.requires_grad, self.model.LTE_NAS.parameters() if 
             args.num_gpu==1 else self.model.module.LTE_NAS.parameters()), 
             "lr": args.lr_rate_lte
            }
        ]
    ...
    def infer(self, current_epoch=0):
        ...
        return psnr_ave, ssim_ave
                #self.logger.info('NAS: Ref  PSNR (now): %.3f \t SSIM (now): %.4f' %(psnr_ave, ssim_ave))
                #if (psnr_ave > self.max_psnr):
                    #self.max_psnr = psnr_ave
                    #self.max_psnr_epoch = current_epoch
                #if (ssim_ave > self.max_ssim):
                    #self.max_ssim = ssim_ave
                    #self.max_ssim_epoch = current_epoch
                #self.logger.info('NAS: Ref  PSNR (max): %.3f (%d) \t SSIM (max): %.4f (%d)' 
                    #%(self.max_psnr, self.max_psnr_epoch, self.max_ssim, self.max_ssim_epoch))
